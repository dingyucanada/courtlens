...
export async function runStoreTests() {
  ...
  try {
    ...
    await run('restore creates a new revision and preserves old history',async()=>{
      const initial=fixture();initial.plays[0].reviewed=true;let p=await a.save(initial,0);const originalName=p.name;p=await a.save({...p,name:'Edited title'},1);p=await b.restore(p.id,1,2);assert(p.revision===3&&p.name===originalName&&p.status==='draft');assert(p.plays.every(play=>play.reviewed===false));const history=await a.history(p.id);assert(history.length===3&&history[1].name==='Edited title');await rejects(()=>a.restore(p.id,1,2),'CONFLICT');
    });
